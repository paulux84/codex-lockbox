...
docker exec --user root "$CONTAINER_NAME" bash -c 'if [ ! -d /codex_home/bin ]; then exit 0; fi; if [ ! -w /usr/local/bin ]; then exit 0; fi; shopt -s nullglob; for exe in /codex_home/bin/*; do [ -x "$exe" ] || continue; target="/usr/local/bin/$(basename "$exe")"; if [ -e "$target" ] && [ ! -w "$target" ]; then continue; fi; ln -sf "$exe" "$target" || true; done; if [ -x /usr/local/share/npm-global/bin/codex ]; then target=/usr/local/bin/codex; if [ -e "$target" ] && [ ! -w "$target" ]; then :; else ln -sf /usr/local/share/npm-global/bin/codex "$target" || true; fi; fi'
